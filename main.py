...
def return_book():
    ...
    if not os.path.exists("librecord.txt"):
        print("Error opening the book records file.")
        return
        ...
        for line in lines:
            ...
            if len(parts) > 5:
                ...
                if current_book_id == book_id:
                    # Check if the book is issued (OUT) and update it to IN
                    if status == 0:  # Assuming '0' means the book is issued
    ...
    os.rename(temp_file, "librecord.txt")

    if found:
        # Update member record file
        if not os.path.exists("member_record.txt"):
            ...
            for line in lines:
                ...
                if len(parts) == 3:
                    ...
                    if current_member_id == member_id:
                        # Update the member's fines if needed
                        fines = max(fines, 0)
...
def issue_book():
    ...
    # Open the library record file for reading and updating
    try:
        with open("librecord.txt", "r") as lib_file:
            lines = lib_file.readlines()
            ...
            for line in lines:
                ...
                if len(parts) >= 5:
                    ...
                    if current_id == book_id:
                        found = True
                        status = int(parts[3])
                        if status == 1:  # Assuming 1 means IN and 0 means OUT
                            status = 0
                        ...
                        # Write the updated record to the temporary file
                        temp_file.write(f"{current_id}\t{parts[1]}\t{parts[2]}\t{status}\t{parts[4]}")
                        temp_file.write('\t'.join(parts[5:]) + '\n')
                    else:
